# ============================================================================
# БЛОК 1: ИМПОРТЫ И НАЧАЛЬНАЯ НАСТРОЙКА
# ============================================================================
import sys
import os
import uuid
...
# ============================================================================
# БЛОК 3: ДЕКОРАТОРЫ И ВСПОМОГАТЕЛЬНЫЕ ФУНКЦИИ
# ============================================================================
...
def create_app(gui_signal=None):
    ...
    def hex_to_rgb_filter(hex_color):
        """Конвертирует HEX в RGB для использования в rgba()"""
        if not hex_color or not hex_color.startswith('#'):
            return '30, 41, 59'  # значение по умолчанию
    ...
    # ============================================================================
    # БЛОК 5: ОСНОВНЫЕ МАРШРУТЫ (ГЛАВНАЯ, ПРОФИЛЬ, РЕГИСТРАЦИЯ, ВХОД)
    # ============================================================================
    ...
    def get_custom_themes():
        """Получить все кастомные темы пользователя"""
    ...
    def save_custom_theme():
        """Сохранить текущую тему как пресет"""
    ...
    # ============================================================================
    # БЛОК 6: РАБОТА С ФАЙЛАМИ (ЗАГРУЗКА, ПРОСМОТР, СКАЧИВАНИЕ, УДАЛЕНИЕ)
    # ============================================================================
    ...
    def upload():
        if request.method == "POST":
            ...
            # Получаем batch_id из формы, если он был передан (для добавления в существующий пакет)
    ...
    # ============================================================================
    # БЛОК 8: ПУБЛИЧНЫЙ ДОСТУП И ШАРИНГ
    # ============================================================================
    ...
    def set_theme():
        ...
        if theme == "custom":
            # Сохраняем ВСЕ настройки кастомной темы
    ...
    def api_chat_history():
        ...
        # Получаем часовой пояс пользователя из заголовка или параметра
        ...
        for m in reversed(messages):
            # Конвертируем UTC время в локальное пользователя
    ...
    @login_required
    def api_dm_list():
        """Получить список личных диалогов"""
        from sqlalchemy import or_, and_
        
        # Находим всех пользователей, с которыми были диалоги
        user_ids = set()
    ...
    # ============================================================================
    # БЛОК 15: ЧАНКОВАЯ ЗАГРУЗКА БОЛЬШИХ ФАЙЛОВ
    # ============================================================================

    

    # Инициализируем менеджер чанков
    ...
    def complete_chunk_upload():
        """Завершение загрузки и сборка файла"""
        data = request.get_json()
    ...
    def get_resume_info(session_id):
        """Получить информацию о загруженных чанках для возобновления"""
    ...
    # ============================================================================
    # БЛОК 16: АВТОМАТИЧЕСКАЯ ОЧИСТКА СТАРЫХ ЧАНКОВ
    # ============================================================================

    import atexit

    def cleanup_chunks_on_exit():
        """Очистка чанков при завершении"""
    ...
    # ============================================================================
    # БЛОК 14: WEBSOCKET ОБРАБОТЧИКИ
    # ============================================================================

    # Глобальное множество для онлайн пользователей
    ...
    def handle_send_message(data):
        if current_user.is_authenticated:
            ...
            user_tz = data.get('timezone', 'Europe/Moscow')  # Получаем часовой пояс от клиента
            ...
            if message:
                try:
                    ...
                    # Конвертируем время в локальное для отправителя
                    ...
                    # Отправляем отправителю в его комнату
                    socketio.emit('message', msg_data, room=f"user_{current_user.id}")
                    
                    # Если личное сообщение - отправляем только получателю
                    if is_private and recipient_id:
                        recipient = User.query.get(recipient_id)
                        if recipient:
                            # Для получателя используем его часовой пояс (если есть в сессии)
                        ...
                        # Общий чат - всем с их часовыми поясами
    ...
    def handle_typing(data):
        if current_user.is_authenticated:
            ...
            # Также отправляем самому отправителю для обратной связи
    ...
    def handle_join_dm_room(data):
        ...
        if current_user.is_authenticated:
            ...
            if other_user_id:
                # Создаем уникальную комнату для диалога
    ...
    # ============================================================================
    # БЛОК 15: ИНИЦИАЛИЗАЦИЯ БАЗЫ ДАННЫХ
    # ============================================================================
